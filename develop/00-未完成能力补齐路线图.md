...
- 状态：`done`
- 本期已完成：
  1. 新增 `verify_release_audit_index.py`，对 `release_audit_index.jsonl` 执行字段契约与与摘要 JSON 一致性校验。
  2. `release-package` 工作流新增索引校验 gate（校验 `release_version/git_commit/sha256/component_count` 等关键字段）。
  3. 发布包新增携带索引校验脚本，并纳入 required-files 验包规则。
  4. develop 文档补齐“摘要 -> 索引 -> 索引校验 -> 发布”闭环说明，形成机器可检索且可校验的发布审计链路。
