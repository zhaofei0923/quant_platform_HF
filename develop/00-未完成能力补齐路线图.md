...
1. `M9`：单机生产基线验收
   - 状态：`done`
   - 交付：单机基线部署/故障注入/回滚/恢复证据链。
   - 证据：`docs/results/deploy_rollback_result.env`、`docs/results/wal_recovery_result.env`、`docs/results/epic_d_operational_drill_report_2026-02-11.md`。
2. `M10`：多机备份与容灾增强（仓库范围）
   - 状态：`done`
   - 交付：多机拓扑、主备切换编排脚本、证据校验脚本与演练报告。
   - 证据：`infra/docker-compose.prodlike.multi-host.yaml`、`scripts/infra/bootstrap_prodlike_multi_host.sh`、`scripts/ops/failover_orchestrator.py`、`scripts/ops/verify_failover_evidence.py`、`docs/results/failover_result.env`、`docs/results/failover_drill_report_2026-02-11.md`。
3. `M2`：单机增强数据链路（Kafka + ClickHouse + Debezium + Parquet 生命周期）
   - 状态：`done`
   - 交付：single-host-m2 部署资产、行情总线回退与回放、ClickHouse 实时/CDC 表结构、Debezium connector、Parquet 分区导出、对象存储生命周期治理、M2 证据校验脚本。
   - 证据：`infra/docker-compose.single-host.m2.yaml`、`infra/clickhouse/init/001_market_realtime.sql`、`infra/clickhouse/init/002_trading_core_cdc.sql`、`infra/debezium/connectors/trading_core.json`、`scripts/ops/replay_market_bus_spool.py`、`scripts/data_pipeline/export_parquet_partitions.py`、`scripts/data_pipeline/run_lifecycle.py`、`scripts/ops/verify_m2_dataflow_evidence.py`。
