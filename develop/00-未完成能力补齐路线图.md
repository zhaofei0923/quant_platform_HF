...
- 状态：`done`
- 本期已完成：
  1. 新增 `release_audit_index.py`，把审计摘要 JSON 生成单行 JSONL 索引（机器可检索）。
  2. `release-package` 工作流新增索引生成步骤，并将 `release_audit_index.jsonl` 作为 artifact 与 tag release 资产上传。
  3. 发布包新增 `scripts/build/release_audit_index.py`，并纳入 required-files 验包规则。
  4. develop 文档补齐“摘要 + 索引”双格式审计闭环说明，形成“生成 -> 校验 -> 索引 -> 发布”链路。
