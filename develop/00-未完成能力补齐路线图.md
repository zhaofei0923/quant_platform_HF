...
- 状态：`done`
- 本期已完成：
  1. `release_audit_summary.py` 增加 `--json-output`，可直接生成结构化审计 JSON。
  2. `release-package` 工作流上传 `release_audit_summary.json`，并在 tag 发布时一并作为 release asset 发布。
  3. 运维 runbook 与 develop 文档补齐 JSON 审计工件命令，形成可查询与可追溯双轨证据。
  4. 保持 `release_manifest`、checksum、组件清单在 Markdown/JSON 两种格式中一致。
