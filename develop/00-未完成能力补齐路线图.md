...
- 状态：`done`
- 本期已完成：
  1. 新增 `verify_release_audit_summary.py`，对审计 JSON 执行必填字段与一致性校验。
  2. `release-package` 工作流新增审计 JSON gate（校验 `release_version/git_commit/component_count` 等关键字段）。
  3. 发布包中新增携带审计 JSON 校验脚本，并纳入 required-files 验包规则。
  4. runbook 与 develop 文档补齐审计 JSON 校验命令，形成“生成 -> 校验 -> 发布”闭环。
