...
- 状态：`done`
- 本期已完成：
  1. 回测新增 `emit_state_snapshots`，研究回放可派发 `StateSnapshot7D` 与实盘输入合约对齐。
  2. CLI 新增 `--emit-state-snapshots` 参数并支持落盘到规范化 `BacktestRunSpec`。
  3. 新增回归测试覆盖状态合约派发路径，防止研究/实盘接口再次漂移。
