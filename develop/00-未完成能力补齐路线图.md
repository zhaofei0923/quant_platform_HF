...
- 状态：`done`
- 本期已完成：
  1. 回测场景模板标准化（`smoke_fast/baseline_replay/deterministic_regression`）。
  2. `BacktestRunSpec` 支持由模板快速构建并保持可复现 run_id 语义。
  3. CLI 新增 `--scenario-template` 入口，减少手工参数漂移并保持 deterministic 可审计。
