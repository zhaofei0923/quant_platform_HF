...
- 状态：`done`
- 本期已完成：
  1. `ReplayReport` 新增数据概览：`instrument_count/instrument_universe/first_ts_ns/last_ts_ns`。
  2. `BacktestRunResult.to_dict()` 输出结构化数据概览字段，便于报告标准化落盘。
  3. 新增多合约与时间范围回归测试，防止数据概览字段回退。
