...
- 状态：`done`
- 本期已完成：
  1. `BacktestRunResult` 新增输入签名与数据签名分离能力（`input_signature/data_signature`）。
  2. 同一 spec 且数据文件内容发生变化时，可通过 `data_signature` 检测数据漂移。
  3. CLI 输出增加 `data_sig` 摘要，便于快速人工比对复现实验证据。
