...
- 当前 Sprint：`B1 / B2 / C1 / D1`（双周）
- 状态：`done`
- 本期已完成：
  1. 风控规则组配置化 + `RiskDecision` 元数据（`rule_group/rule_version/decision_ts_ns`）。
  2. 执行模式 `direct|sliced` 与切片参数（`slice_size/slice_interval_ms`）。
  3. Ops 健康报告（JSON + Markdown）与 CI smoke（`run_reconnect_evidence --dry-run`）。
  4. 发布包版本规则、release manifest、回滚演练与 WAL RTO/RPO 证据模板。
