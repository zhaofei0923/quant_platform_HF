...
- 状态：`done`
- 本期已完成：
  1. 新增 `scripts/build/verify_nonhotpath_release.py`，对发布包执行 checksum + manifest + 必需文件校验。
  2. `release-package` 工作流新增自动验包步骤，并修复手动触发版本号非法问题（非 tag 时使用 UTC 时间戳）。
  3. 发布包中新增携带验包脚本，便于交付后离线验证。
  4. systemd/k8s runbook 与 develop 对齐文档同步补充验包命令与证据路径。
