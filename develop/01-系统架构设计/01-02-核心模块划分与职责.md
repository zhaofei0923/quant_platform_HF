...
**职责描述：**
基于ZeroMQ封装高性能网络通信框架，提供进程间通信（IPC）和跨机通信能力。管理连接池、消息序列化、负载均衡和故障恢复。

**核心功能：**
- **连接池管理：** 自动连接建立、维护、回收和负载均衡
- **消息序列化：** Protocol Buffers序列化/反序列化，支持零拷贝优化
- **通信模式：** 支持PUB/SUB、REQ/REP、PUSH/PULL等多种模式
- **故障检测：** 心跳检测、超时管理、自动重连
- **流量控制：** 背压控制、流量整形、优先级队列
...
public:
    // 连接管理
    ConnectionHandle createConnection(const string& endpoint, ConnectionType type);
    bool closeConnection(ConnectionHandle handle);
    
    // 消息发送（异步）
    Future<SendResult> sendAsync(ConnectionHandle handle, const Message& msg, 
                                 Priority priority = Priority::NORMAL);
    
    // 消息接收（事件驱动）
    void registerMessageCallback(ConnectionHandle handle, MessageCallback cb);
    
    // 批量操作
    vector<Future<SendResult>> sendBatch(ConnectionHandle handle, 
                                         const vector<Message>& msgs);
    
    // 连接状态监控
    ConnectionStatus getConnectionStatus(ConnectionHandle handle) const;
    vector<ConnectionStats> getConnectionStats() const;
    
    // 流量控制
    void setRateLimit(ConnectionHandle handle, size_t messagesPerSecond);
...
public:
    ...
    template<typename T>
    static T deserialize(const vector<uint8_t>& data);
    
    // 零拷贝序列化（避免内存拷贝）
    template<typename T>
    static void serializeZeroCopy(const T& obj, vector<uint8_t>& buffer);
};
```

**性能要求：**
- **IPC延迟：** P99 < 50μs（本地进程间通信）
- **网络延迟：** P99 < 200μs（同机房跨机通信）
- **吞吐量：** > 100万消息/秒（单连接）
- **连接建立时间：** < 10ms
- **故障检测时间：** < 100ms（心跳超时检测）
- **重连时间：** < 500ms（包含状态恢复）
