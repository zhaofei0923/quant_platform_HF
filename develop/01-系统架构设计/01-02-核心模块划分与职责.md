...
| 模块 | 目标职责 | 触发条件 | 当前状态 |
|---|---|---|---|
| 高级风险规则引擎 | 支持多规则组、可解释决策和策略化配置 | 风险规则模型冻结 + 回放验收基线稳定 | 已落地（仓库范围） |
| 高级执行算法层 | TWAP/VWAP/自适应切片等执行策略 | 订单事件字段与回放模型冻结 | 已落地（仓库范围） |
| 统一观测控制面 | 指标、日志、告警、演练证据统一视图 | 指标命名与SLO口径统一 | 已落地（仓库范围） |
| 集群级部署控制面 | 多环境发布编排、灰度与自动回滚 | 非热路径部署链路稳定、环境托管策略明确 | 已落地（仓库范围） |
