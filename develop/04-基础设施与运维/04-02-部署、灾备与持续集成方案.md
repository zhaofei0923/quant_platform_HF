...
- 实现状态：部分落地
- 证据路径：`.github/workflows/ci.yml`、`.github/workflows/release-package.yml`、`scripts/build/package_nonhotpath_release.sh`、`scripts/build/verify_nonhotpath_release.py`、`docs/SYSTEMD_DEPLOYMENT_RUNBOOK.md`、`docs/K8S_DEPLOYMENT_RUNBOOK.md`
- 最后更新：2026-02-11
...
- 发布流程：`.github/workflows/release-package.yml`（打包 + 自动验包 + 工件上传）。
- 部署渲染：`scripts/ops/render_systemd_units.py`、`scripts/ops/render_k8s_manifests.py`。
- 交付打包与验包：`scripts/build/package_nonhotpath_release.sh`、`scripts/build/verify_nonhotpath_release.py`。
