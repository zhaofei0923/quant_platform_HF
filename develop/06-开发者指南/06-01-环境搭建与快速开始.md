...
- 实现状态：已落地
- 证据路径：`scripts/build/bootstrap.sh`、`src/apps/core_engine_main.cpp`、`scripts/strategy/run_strategy.py`
- 最后更新：2026-02-11
...
### 5.2 构建并运行 core_engine（外部 Redis + Timescale 模式）

```bash
# Ubuntu 目标机建议使用原生 Docker Engine（docker-ce），不依赖 Docker Desktop。
cmake -S . -B build \
...
export QUANT_HFT_TIMESCALE_MODE=external
export QUANT_HFT_TIMESCALE_HOST=127.0.0.1
export QUANT_HFT_TIMESCALE_PORT=5432
export QUANT_HFT_TIMESCALE_DB=quant_hft
export QUANT_HFT_TIMESCALE_USER=quant_hft
export QUANT_HFT_TIMESCALE_PASSWORD=quant_hft
export QUANT_HFT_STORAGE_ALLOW_FALLBACK=false

./build/core_engine configs/sim/ctp.yaml --run-seconds 30
```

`configs/sim/ctp.yaml` 中执行策略相关键（节选）：
- `execution_mode`: `direct` / `sliced`
- `slice_size` / `slice_interval_ms`
- `cancel_after_ms`（`0` 为关闭超时撤单）
