...
- 实现状态：规划中
- 证据路径：`scripts/ops/ctp_preflight_check.py`、`scripts/ops/run_reconnect_evidence.py`、`docs/WAL_RECOVERY_RUNBOOK.md`
- 最后更新：2026-02-11
...
```bash
.venv/bin/python scripts/ops/run_reconnect_evidence.py   --config configs/sim/ctp.yaml   --output docs/results/reconnect_fault_result.md
```

### 3. SLO 报告生成

```bash
.venv/bin/python scripts/ops/reconnect_slo_report.py   --input docs/results/reconnect_fault_result.md
```
