#!/usr/bin/env bash
set -u
cd /tmp/quant-hft-stage2-snapshot
evidence=/tmp/quant-hft-stage2-smoke
engine=/tmp/quant-hft-stage2-build/core_engine
run_engine() {
    env -i PATH=/usr/bin:/bin HOME="$evidence/home" TZ=UTC \
        ...
        RISK_RULE_FILE_PATH=/tmp/quant-hft-stage2-snapshot/configs/risk_rules.yaml \
