...
class LibpqTimescaleSqlClient : public ITimescaleSqlClient {
   public:
    ...
    bool LockTransactionKey(const std::string& key, std::string* error) override;
   ...
   private:
    ...
    bool ValidateSimpleIdentifier(const std::string& identifier, const std::string& field_name,
                                  std::string* error) const;
    bool ValidateQualifiedTableIdentifier(const std::string& table_identifier,
                                          std::string* quoted_identifier, std::string* error) const;
    ...
    static std::vector<std::unordered_map<std::string, std::string>> ParseRows(const LibpqApi& api,
                                                                               void* result_ptr);
    ...
    void* transaction_connection_{nullptr};
};
