...
class OrderManager {
   public:
    ...
    bool OnOrderEvent(const OrderEvent& event, Order* out_order, std::string* error,
                      bool* transition_rejected = nullptr);
    bool OnTradeEvent(const OrderEvent& event, Trade* out_trade, std::string* error);
    Trade BuildTrade(const OrderEvent& event) const;

    std::optional<Order> GetOrder(const std::string& client_order_id) const;
    std::vector<Order> GetActiveOrders() const;
    std::vector<Order> GetActiveOrdersByAccount(const std::string& account_id,
                                                const std::string& instrument_id = "") const;
    std::vector<Order> GetActiveOrdersByStrategy(const std::string& strategy_id,
                                                 const std::string& instrument_id = "") const;
   ...
   private:
    bool IsEventProcessed(const std::string& event_key) const;
    bool MarkEventProcessed(const std::string& event_key, const OrderEvent& event,
