...
int main() {
    try {
        const std::map<std::string, std::vector<std::string>> expected_fields = {
            ...
            {"MarketSnapshot", {"instrument_id",       "exchange_id",
                                "trading_day",         "action_day",
                                "update_time",         "update_millisec",
                                "last_price",          "bid_price_1",
                                "ask_price_1",         "bid_volume_1",
                                "ask_volume_1",        "volume",
                                "open_interest",       "settlement_price",
                                "average_price_raw",   "average_price_norm",
                                "is_valid_settlement", "exchange_ts_ns",
                                "recv_ts_ns",          "average_price_norm_valid"}},
            {"StateSnapshot7D",
             {"instrument_id",
              "trend",
              "volatility",
            ...
            {"SignalIntent",
             {"strategy_id", "instrument_id", "signal_type", "side", "offset", "volume",
              "limit_price", "ts_ns", "trace_id", "generated_ts_ns"}},
            {"RiskDecision",
            ...
            {"OrderIntent",
             {"account_id", "client_order_id", "strategy_id", "instrument_id", "side", "offset",
              ...
              "ts_ns", "trace_id", "exchange_id", "trading_day", "signal_ts_ns",
              "market_recv_ts_ns"}},
