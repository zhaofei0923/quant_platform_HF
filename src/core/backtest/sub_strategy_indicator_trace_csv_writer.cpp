...
bool SubStrategyIndicatorTraceCsvWriter::Open(const std::string& output_path, std::string* error) {
    if (is_open_) {
        return SetError("sub-strategy indicator trace csv writer is already open", error);
    }
    if (output_path.empty()) {
        return SetError("sub-strategy indicator trace csv output path is empty", error);
    }
    ...
    try {
        ...
        return SetError(
            std::string("failed to prepare sub-strategy indicator trace csv path: ") + ex.what(),
            error);
    }

    out_.open(output_path, std::ios::out | std::ios::trunc);
    if (!out_.is_open()) {
        return SetError("failed to open sub-strategy indicator trace csv output: " + output_path,
                        error);
    }
...
bool SubStrategyIndicatorTraceCsvWriter::Append(const SubStrategyIndicatorTraceRow& row,
                                                std::string* error) {
    if (!is_open_) {
        return SetError("sub-strategy indicator trace csv writer is not open", error);
    }
...
bool SubStrategyIndicatorTraceCsvWriter::Close(std::string* error) {
    if (!is_open_) {
        return true;
    }
    out_.flush();
    if (!out_.good()) {
        out_.close();
        is_open_ = false;
        return SetError("failed to flush sub-strategy indicator trace csv output", error);
    }
    out_.close();
    is_open_ = false;
    return true;
