...
std::shared_ptr<MonitoringCounter> DispatcherDroppedCounter() {
    static auto counter = MetricRegistry::Instance().BuildCounter(
        "quant_hft_callback_dispatcher_dropped_total", "Total dropped tasks in CallbackDispatcher");
    return counter;
}

std::shared_ptr<MonitoringCounter> DispatcherCriticalTimeoutCounter() {
    static auto counter = MetricRegistry::Instance().BuildCounter(
        "quant_hft_callback_dispatcher_critical_timeout_total",
        "Total critical task enqueue timeouts in CallbackDispatcher");
    return counter;
}

std::shared_ptr<MonitoringCounter> DispatcherCriticalDelayExceededCounter() {
    static auto counter = MetricRegistry::Instance().BuildCounter(
        "quant_hft_callback_dispatcher_critical_queue_delay_exceeded_total",
...
CallbackDispatcher::CallbackDispatcher(std::size_t max_queue_size, std::int64_t critical_wait_ms,
...
CallbackDispatcher::~CallbackDispatcher() { Stop(); }
...
void CallbackDispatcher::Start() {
    ...
    stop_ = false;
    worker_ = std::thread(&CallbackDispatcher::WorkerLoop, this);
    running_ = true;
}
...
bool CallbackDispatcher::Post(Task task, bool is_critical) {
    if (!task) {
        return false;
    }
    ...
    if (queue_.size() >= max_queue_size_) {
        if (!is_critical) {
            ...
            DispatcherDroppedCounter()->Increment();
            EmitStructuredLog(
                nullptr, "callback_dispatcher", "warn", "queue_full",
                {{"is_critical", "false"},
                 {"queue_depth", std::to_string(queue_.size())},
                 {"queue_capacity", std::to_string(max_queue_size_)},
                 {"dropped_total", std::to_string(dropped_.load(std::memory_order_relaxed))},
                 {"action", "drop"}});
            return false;
        }
        const bool wait_ok =
            space_cv_.wait_for(lock, std::chrono::milliseconds(critical_wait_ms_), has_space);
        if (!wait_ok || queue_.size() >= max_queue_size_) {
            dropped_.fetch_add(1, std::memory_order_relaxed);
            critical_timeout_.fetch_add(1, std::memory_order_relaxed);
            DispatcherDroppedCounter()->Increment();
            DispatcherCriticalTimeoutCounter()->Increment();
            EmitStructuredLog(
                nullptr, "callback_dispatcher", "error", "queue_full",
                {{"is_critical", "true"},
                 {"queue_depth", std::to_string(queue_.size())},
                 {"queue_capacity", std::to_string(max_queue_size_)},
                 {"dropped_total", std::to_string(dropped_.load(std::memory_order_relaxed))},
...
CallbackDispatcher::Stats CallbackDispatcher::GetStats() const {
    std::lock_guard<std::mutex> lock(mutex_);
    Stats stats;
...
void CallbackDispatcher::WorkerLoop() {
    while (true) {
        ...
        if (queued_task.is_critical) {
            ...
            if (delay_ms > critical_delay_alert_ms_) {
                ...
                DispatcherCriticalDelayExceededCounter()->Increment();
                EmitStructuredLog(
                    nullptr, "callback_dispatcher", "warn", "critical_queue_delay",
        ...
        Task queued = std::move(queued_task.task);
        if (!queued) {
            continue;
        }
        queued();
