...
class BacktestDataFeedTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        temp_dir_ =
            fs::temp_directory_path() / ("backtest_data_feed_test_" + std::to_string(stamp));
        ...
        ASSERT_TRUE(test::WriteTickPartitionFixture(parquet_file_,
                                                    {
                                                        Tick{.symbol = "rb2405",
                                                             .exchange = "SHFE",
...
TEST_F(BacktestDataFeedTest, RunInvokesTickCallbackAndAdvancesCurrentTime) {
    ...
    feed.Subscribe(
        {"rb2405"}, [&](const Tick&) { ++callback_count; }, nullptr);
