...
TEST(RunBacktestWithValidationScriptTest, HelpShowsPortfolioCandidateDefaultConfig) {
    const auto root = MakeTempDir("help_default_config");
    const auto log_file = root / "help.log";

    const std::string command =
        "/bin/bash scripts/build/run_backtest_with_validation.sh --help >'" +
        EscapePathForShell(log_file) + "' 2>&1";
    const int rc = RunCommand(command);
    EXPECT_EQ(rc, 0);

    const std::string payload = ReadFile(log_file);
    EXPECT_NE(payload.find("configs/backtest/backtest_run_sim_trade_candidates.yaml"),
              std::string::npos);
}

void WriteFakeBacktestCli(const std::filesystem::path& path,
                          const std::filesystem::path& args_log) {
    WriteExecutable(
        path,
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        "args_file='" +
            ...
            "printf '# backtest\\n' >\"${output_md}\"\n"
            "printf "
            "'date,capital,daily_return_pct,cumulative_return_pct,drawdown_pct,position_value,"
            "trades_count,turnover,market_regime\\n' >\"${export_csv_dir}/daily_equity.csv\"\n"
            "printf "
            "'fill_seq,trade_id,order_id,symbol,side,offset,volume,price,timestamp_ns,timestamp_dt_"
            "local,commission,slippage,realized_pnl,strategy_id,signal_type,regime_at_entry,risk_"
...
TEST(RunBacktestWithValidationScriptTest, FastModeDisablesHeavyOutputsAndSkipsFormalReport) {
    ...
    const auto config_path =
        WriteConfig(root, build_dir, dataset_root, strategy_main, output_root_dir);
    ...
    const std::string command =
        ...
        "' --fast --fast-start-date 20240201 --fast-end-date 20240205 "
        "--fast-max-ticks 321 --skip-build >'" +
        EscapePathForShell(log_file) + "' 2>&1";
    const int rc = RunCommand(command);
...
TEST(RunBacktestWithValidationScriptTest, CustomRunIdUsesLatestRunDirectory) {
    const auto root = MakeTempDir("custom_run_id");
    const auto fake_bin_dir = root / "fake-bin";
    const auto build_dir = root / "build-gcc";
    const auto dataset_root = root / "parquet_v2";
    const auto strategy_main = root / "main_backtest_strategy.yaml";
    ...
    WriteFakeBacktestCli(build_dir / "backtest_cli", args_log);
    WriteFakePython(fake_bin_dir / "python3", python_log);
    const auto config_path = WriteConfig(root, build_dir, dataset_root, strategy_main,
                                         output_root_dir, "sim_trade_candidates_c_rb_m_hc");
    ...
    const std::string payload = ReadFile(log_file);
    EXPECT_NE(payload.find("sim_trade_candidates_c_rb_m_hc_"), std::string::npos);
    EXPECT_NE(payload.find("validation_report.md"), std::string::npos);
}
