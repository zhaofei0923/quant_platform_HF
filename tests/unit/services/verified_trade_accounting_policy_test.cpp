...
std::string ValidGenericCloseConventionJson() {
    return R"json({"schema_version":1,"environment":"simnow","conventions":[
{"exchange_id":"DCE","generic_close_priority":"yesterday_first",
"evidence_source":"reviewed_ctp_close_convention","evidence_version":"2026-09-07",
"evidence_sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"},
{"exchange_id":"GFEX","generic_close_priority":"today_first",
"evidence_source":"reviewed_ctp_close_convention","evidence_version":"2026-09-07",
...
class VerifiedTradeAccountingPolicyTest : public ::testing::Test {
   protected:
    ...
    PolicyEvidence MakeBrokerObservedEvidence(const std::string& instrument_id,
                                              const std::string& product_id,
                                              const std::string& exchange_id) const {
        PolicyEvidence evidence;
        evidence.instrument.metadata.instrument_id = instrument_id;
        evidence.instrument.metadata.query_name = "Instrument";
        evidence.instrument.rows[0].instrument_id = instrument_id;
        evidence.instrument.rows[0].exchange_id = exchange_id;
        evidence.instrument.rows[0].product_id = product_id;
        evidence.instrument.rows[0].volume_multiple = 10;
        evidence.commission.metadata.instrument_id = instrument_id;
        evidence.commission.metadata.query_name = "InstrumentCommissionRate";
        evidence.commission.rows[0].instrument_id = product_id;
        evidence.commission.rows[0].exchange_id = exchange_id;
        evidence.order_commission.metadata.instrument_id = instrument_id;
        evidence.order_commission.metadata.query_name = "InstrumentOrderCommRate";
        evidence.order_commission.rows[0].instrument_id = instrument_id;
        evidence.order_commission.rows[0].exchange_id = exchange_id;
        return evidence;
    }

    PolicyEvidence MakeHcEvidence() const {
        return MakeBrokerObservedEvidence("hc2701", "hc", "SHFE");
    }

    Trade MatchingBrokerObservedTrade(const std::string& instrument_id,
                                      const std::string& exchange_id) const {
        auto trade = MatchingTrade();
        trade.symbol = instrument_id;
        trade.exchange = exchange_id;
        return trade;
    }

    Trade MatchingHcTrade() const {
        return MatchingBrokerObservedTrade("hc2701", "SHFE");
    }

    SimNowGenericCloseConvention GenericConvention(
        const std::string& exchange_id,
        GenericClosePriority priority = GenericClosePriority::kYesterdayFirst) const {
        return {exchange_id, priority, "reviewed_ctp_close_convention", "2026-09-07",
...
TEST_F(VerifiedTradeAccountingPolicyTest,
       GenericCloseConventionFileLoaderAcceptsOnlyStrictControlledArtifacts) {
    const auto valid_file = dir / "generic_close.json";
    std::ofstream(valid_file) << ValidGenericCloseConventionJson();
    std::vector<SimNowGenericCloseConvention> conventions;
    std::string error;
    ASSERT_TRUE(LoadSimNowGenericCloseConventionsFromFile(valid_file.string(), &conventions,
                                                          &error))
        << error;
    ASSERT_EQ(conventions.size(), 2U);
    EXPECT_EQ(conventions[0].exchange_id, "DCE");
    EXPECT_EQ(conventions[0].priority, GenericClosePriority::kYesterdayFirst);
    EXPECT_EQ(conventions[1].exchange_id, "GFEX");
    EXPECT_EQ(conventions[1].priority, GenericClosePriority::kTodayFirst);

    std::error_code symlink_error;
    const auto symlink = dir / "generic_close_symlink.json";
    std::filesystem::create_symlink(valid_file, symlink, symlink_error);
    ASSERT_FALSE(symlink_error) << symlink_error.message();
    EXPECT_FALSE(
        LoadSimNowGenericCloseConventionsFromFile(symlink.string(), &conventions, &error));
    EXPECT_EQ(conventions.size(), 2U);
    EXPECT_FALSE(LoadSimNowGenericCloseConventionsFromFile("relative.json", &conventions,
                                                           &error));
    EXPECT_FALSE(LoadSimNowGenericCloseConventionsFromFile(dir.string(), &conventions, &error));
    EXPECT_FALSE(LoadSimNowGenericCloseConventionsFromFile(
        (dir / "missing.json").string(), &conventions, &error));
    EXPECT_FALSE(
        LoadSimNowGenericCloseConventionsFromFile(valid_file.string(), nullptr, &error));

    const auto oversized = dir / "oversized.json";
    std::ofstream(oversized) << std::string(64U * 1024U + 1U, 'x');
    EXPECT_FALSE(
        LoadSimNowGenericCloseConventionsFromFile(oversized.string(), &conventions, &error));
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       GenericCloseConventionFileLoaderRejectsSchemaAndRecordAmbiguity) {
    const auto valid = ValidGenericCloseConventionJson();
    const std::vector<std::string> malformed = {
        Replace(valid, "\"schema_version\":1", "\"schema_version\":2"),
        Replace(valid, "\"environment\":\"simnow\"", "\"environment\":\"prod\""),
        Replace(valid, "\"schema_version\":1", "\"unknown\":1,\"schema_version\":1"),
        Replace(valid, "\"exchange_id\":\"DCE\"", "\"exchange_id\":\"CFFEX\""),
        Replace(valid, "\"generic_close_priority\":\"yesterday_first\"",
                "\"generic_close_priority\":\"guess\""),
        Replace(valid, "\"evidence_version\":\"2026-09-07\"",
                "\"evidence_version\":\"\""),
        Replace(valid,
                "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
                "not-a-sha256"),
        Replace(valid, "\"exchange_id\":\"GFEX\"", "\"exchange_id\":\"DCE\""),
    };
    for (std::size_t i = 0; i < malformed.size(); ++i) {
        SCOPED_TRACE(i);
        const auto file = dir / ("invalid_" + std::to_string(i) + ".json");
        std::ofstream(file) << malformed[i];
        std::vector<SimNowGenericCloseConvention> conventions = {GenericConvention("CZCE")};
        std::string error;
        EXPECT_FALSE(
            LoadSimNowGenericCloseConventionsFromFile(file.string(), &conventions, &error));
        EXPECT_EQ(conventions.size(), 1U);  // A failed parse does not replace the active gate.
        EXPECT_FALSE(error.empty());
    }
}
...
TEST_F(VerifiedTradeAccountingPolicyTest, BrokerObservedModeIsStrictlySimNowOnly) {
    for (const auto* environment : {"prod", "sim", "live", "SIMNOW"}) {
        SCOPED_TRACE(environment);
        auto candidate = identity;
        candidate.environment = environment;
        VerifiedTradeAccountingPolicyRegistry registry;
        std::string error;
        EXPECT_FALSE(registry.EnableSimNowBrokerObservedMode(candidate, {}, &error));
        EXPECT_FALSE(error.empty());
    }
    for (int missing = 0; missing < 2; ++missing) {
        auto candidate = identity;
        if (missing == 0) candidate.broker_id.clear();
        if (missing == 1) candidate.account_id.clear();
        VerifiedTradeAccountingPolicyRegistry registry;
        std::string error;
        EXPECT_FALSE(registry.EnableSimNowBrokerObservedMode(candidate, {}, &error));
    }
    VerifiedTradeAccountingPolicyRegistry registry;
    std::string error;
    EXPECT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, {}, &error)) << error;
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       BrokerObservedModeDerivesValuationFromExactSameSessionCtpFacts) {
    const AccountingPolicyInstrument hc{"hc2701", "SHFE", HedgeFlag::kSpeculation};
    auto evidence = MakeHcEvidence();
    VerifiedTradeAccountingPolicyRegistry registry;
    std::string error;
    ASSERT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, {}, &error)) << error;
    registry.BeginSession(3);
    evidence.Observe(&registry);
    ASSERT_TRUE(registry.ReadyFor({hc}, kDay, &error)) << error;
    const auto policy = registry.Resolve(MatchingHcTrade(), &error);
    ASSERT_TRUE(policy.valuation_inputs_verified) << error;
    EXPECT_DOUBLE_EQ(policy.contract_multiplier, 10);
    EXPECT_DOUBLE_EQ(policy.open_fee.by_money, 0.001);
    EXPECT_DOUBLE_EQ(policy.close_fee.by_volume, 3);
    EXPECT_DOUBLE_EQ(policy.close_today_fee.by_volume, 4);
    EXPECT_EQ(policy.generic_close_priority, GenericClosePriority::kUnspecified);
    EXPECT_NE(policy.valuation_source.find("order_fee=ctp_verified_zero_v1"),
              std::string::npos);
    EXPECT_NE(policy.valuation_source.find("simnow-broker-observed-v2"),
              std::string::npos);
    EXPECT_NE(policy.valuation_source.find("fee_exchange=exact_instrument_query_anchor"),
              std::string::npos);
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       BrokerObservedModeAnchorsOmittedFeeExchangeToExactInstrumentQuery) {
    const AccountingPolicyInstrument hc{"hc2701", "SHFE", HedgeFlag::kSpeculation};
    auto evidence = MakeHcEvidence();
    evidence.commission.rows[0].exchange_id.clear();
    evidence.order_commission.rows[0].exchange_id.clear();
    ...
    auto conflict = MakeHcEvidence();
    conflict.commission.rows[0].exchange_id = "DCE";
    VerifiedTradeAccountingPolicyRegistry rejected;
    ASSERT_TRUE(rejected.EnableSimNowBrokerObservedMode(identity, {}, &error)) << error;
    rejected.BeginSession(3);
    conflict.Observe(&rejected);
    EXPECT_FALSE(rejected.ReadyFor({hc}, kDay, &error));
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       AssumedZeroOrderFeesRequireExplicitFlagAndRemainDurablyMarked) {
    const AccountingPolicyInstrument hc{"hc2701", "SHFE", HedgeFlag::kSpeculation};
    auto evidence = MakeHcEvidence();
    VerifiedTradeAccountingPolicyRegistry blocked;
    std::string error;
    ASSERT_TRUE(blocked.EnableSimNowBrokerObservedMode(identity, {}, &error));
    blocked.BeginSession(3);
    evidence.Observe(&blocked, 2);
    EXPECT_FALSE(blocked.ReadyFor({hc}, kDay, &error));

    SimNowBrokerObservedAccountingOptions options;
    options.allow_assumed_zero_order_fees = true;
    VerifiedTradeAccountingPolicyRegistry registry;
    ASSERT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, options, &error));
    registry.BeginSession(3);
    evidence.Observe(&registry, 2);
    ASSERT_TRUE(registry.ReadyFor({hc}, kDay, &error)) << error;
    const auto policy = registry.Resolve(MatchingHcTrade(), &error);
    ASSERT_TRUE(policy.valuation_inputs_verified) << error;
    EXPECT_NE(policy.valuation_source.find("order_fee=simnow_assumed_zero_v1"),
              std::string::npos);

    VerifiedTradeAccountingPolicyRegistry::EvidenceState saved;
    ASSERT_TRUE(registry.ExportValidatedEvidence(&saved, &error)) << error;
    EXPECT_EQ(saved.at("schema_version"), "3");
    EXPECT_EQ(saved.at("count"), "1");

    VerifiedTradeAccountingPolicyRegistry restored;
    ASSERT_TRUE(restored.EnableSimNowBrokerObservedMode(identity, options, &error));
    ASSERT_TRUE(restored.RestoreValidatedEvidence(saved, &error)) << error;
    EXPECT_TRUE(restored.ResolveHistorical(MatchingHcTrade(), &error).valuation_inputs_verified)
        << error;
    EXPECT_FALSE(restored.Resolve(MatchingHcTrade(), &error).valuation_inputs_verified);

    VerifiedTradeAccountingPolicyRegistry missing_flag;
    ASSERT_TRUE(missing_flag.EnableSimNowBrokerObservedMode(identity, {}, &error));
    EXPECT_FALSE(missing_flag.RestoreValidatedEvidence(saved, &error));
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       ExplicitAssumptionNeverMasksFailedMismatchedOrNonzeroOrderFeeEvidence) {
    const AccountingPolicyInstrument hc{"hc2701", "SHFE", HedgeFlag::kSpeculation};
    SimNowBrokerObservedAccountingOptions options;
    options.allow_assumed_zero_order_fees = true;
    for (int fault = 0; fault < 6; ++fault) {
        SCOPED_TRACE(fault);
        auto evidence = MakeHcEvidence();
        if (fault == 0) evidence.order_commission.metadata.success = false;
        if (fault == 1) evidence.order_commission.metadata.complete = false;
        if (fault == 2) evidence.order_commission.metadata.account_id = "OTHER_ACCOUNT";
        if (fault == 3) evidence.order_commission.metadata.query_name = "InstrumentCommissionRate";
        if (fault == 4) evidence.order_commission.rows[0].order_comm_by_volume = 0.01;
        if (fault == 5) evidence.order_commission.rows[0].order_action_comm_by_volume = 0.01;
        VerifiedTradeAccountingPolicyRegistry registry;
        std::string error;
        ASSERT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, options, &error));
        registry.BeginSession(3);
        evidence.Observe(&registry);
        EXPECT_FALSE(registry.ReadyFor({hc}, kDay, &error));
        EXPECT_FALSE(registry.Resolve(MatchingHcTrade(), &error).valuation_inputs_verified);
    }

    auto empty = MakeHcEvidence();
    empty.order_commission.rows.clear();
    VerifiedTradeAccountingPolicyRegistry registry;
    std::string error;
    ASSERT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, options, &error));
    registry.BeginSession(3);
    empty.Observe(&registry);
    EXPECT_TRUE(registry.ReadyFor({hc}, kDay, &error)) << error;
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       BrokerObservedModeRejectsNonExactUngatedOrUnsupportedExchangeEvidence) {
    const AccountingPolicyInstrument hc{"hc2701", "SHFE", HedgeFlag::kSpeculation};
    SimNowBrokerObservedAccountingOptions options;
    options.allow_assumed_zero_order_fees = true;
    const std::vector<std::function<void(PolicyEvidence&)>> faults = {
        [](auto& e) { e.instrument.metadata.full_account = true; },
        [](auto& e) { e.instrument.metadata.query_name = "InstrumentCommissionRate"; },
        [](auto& e) { e.instrument.metadata.account_id = "OTHER_ACCOUNT"; },
        [](auto& e) { e.commission.metadata.trading_day = "20260907"; },
        [](auto& e) { e.commission.rows[0].investor_id = "OTHER_ACCOUNT"; },
        [](auto& e) { e.commission.rows[0].instrument_id = "rb"; },
        [](auto& e) { e.instrument.rows.push_back(e.instrument.rows.front()); }};
    for (std::size_t fault = 0; fault < faults.size(); ++fault) {
        SCOPED_TRACE(fault);
        auto evidence = MakeHcEvidence();
        faults[fault](evidence);
    ...
    const std::vector<std::tuple<std::string, std::string, std::string>> ungated = {
        {"m2701", "m", "DCE"},
        {"MA701", "MA", "CZCE"},
        {"si2701", "si", "GFEX"},
        {"IF2701", "IF", "CFFEX"},
    };
    for (const auto& [instrument_id, product_id, exchange_id] : ungated) {
        SCOPED_TRACE(exchange_id);
        auto evidence = MakeBrokerObservedEvidence(instrument_id, product_id, exchange_id);
        const AccountingPolicyInstrument scope{instrument_id, exchange_id,
        ...
        registry.BeginSession(3);
        evidence.Observe(&registry);
        EXPECT_FALSE(registry.ReadyFor({scope}, kDay, &error));
        EXPECT_FALSE(registry
                         .Resolve(MatchingBrokerObservedTrade(instrument_id, exchange_id), &error)
                         .valuation_inputs_verified);
    }
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       GenericCloseConventionConfigurationIsStrictAndFailClosed) {
    std::vector<SimNowBrokerObservedAccountingOptions> invalid;
    for (const auto* exchange_id : {"SHFE", "INE", "CFFEX", "dce", "UNKNOWN"}) {
        SimNowBrokerObservedAccountingOptions options;
        options.generic_close_conventions.push_back(GenericConvention(exchange_id));
        invalid.push_back(std::move(options));
    }
    {
        SimNowBrokerObservedAccountingOptions options;
        auto convention = GenericConvention("DCE");
        convention.priority = GenericClosePriority::kUnspecified;
        options.generic_close_conventions.push_back(std::move(convention));
        invalid.push_back(std::move(options));
    }
    {
        SimNowBrokerObservedAccountingOptions options;
        auto convention = GenericConvention("DCE");
        convention.priority = static_cast<GenericClosePriority>(99);
        options.generic_close_conventions.push_back(std::move(convention));
        invalid.push_back(std::move(options));
    }
    for (int field = 0; field < 3; ++field) {
        SimNowBrokerObservedAccountingOptions options;
        auto convention = GenericConvention("DCE");
        if (field == 0) convention.evidence_source.clear();
        if (field == 1) convention.evidence_version.clear();
        if (field == 2) convention.evidence_sha256 = "not-a-sha256";
        options.generic_close_conventions.push_back(std::move(convention));
        invalid.push_back(std::move(options));
    }
    {
        SimNowBrokerObservedAccountingOptions options;
        options.generic_close_conventions = {GenericConvention("DCE"),
                                             GenericConvention("DCE")};
        invalid.push_back(std::move(options));
    }
    for (std::size_t i = 0; i < invalid.size(); ++i) {
        ...
        VerifiedTradeAccountingPolicyRegistry registry;
        std::string error;
        EXPECT_FALSE(registry.EnableSimNowBrokerObservedMode(identity, invalid[i], &error));
        EXPECT_FALSE(error.empty());
    }
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       ReviewedGenericCloseConventionsEnableExactDceCzceAndGfexScopesTogether) {
    struct TestCase {
        std::string instrument_id;
        std::string product_id;
        std::string exchange_id;
        GenericClosePriority priority;
    };
    const std::vector<TestCase> cases = {
        {"m2701", "m", "DCE", GenericClosePriority::kYesterdayFirst},
        {"MA701", "MA", "CZCE", GenericClosePriority::kTodayFirst},
        {"si2701", "si", "GFEX", GenericClosePriority::kYesterdayFirst},
    };
    SimNowBrokerObservedAccountingOptions options;
    for (const auto& item : cases)
        options.generic_close_conventions.push_back(
            GenericConvention(item.exchange_id, item.priority));

    VerifiedTradeAccountingPolicyRegistry registry;
    std::string error;
    ASSERT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, options, &error)) << error;
    registry.BeginSession(3);
    std::vector<AccountingPolicyInstrument> scopes;
    for (const auto& item : cases)
        scopes.push_back({item.instrument_id, item.exchange_id, HedgeFlag::kSpeculation});
    for (std::size_t i = 0; i < cases.size(); ++i) {
        const auto& item = cases[i];
        auto evidence =
            MakeBrokerObservedEvidence(item.instrument_id, item.product_id, item.exchange_id);
        evidence.instrument.metadata.request_id += static_cast<int>(i * 3);
        evidence.commission.metadata.request_id += static_cast<int>(i * 3 + 1);
        evidence.order_commission.metadata.request_id += static_cast<int>(i * 3 + 2);
        evidence.Observe(&registry);
        if (i + 1 != cases.size()) EXPECT_FALSE(registry.ReadyFor(scopes, kDay, &error));
    }
    ASSERT_TRUE(registry.ReadyFor(scopes, kDay, &error)) << error;
    for (const auto& item : cases) {
        const auto policy = registry.Resolve(
            MatchingBrokerObservedTrade(item.instrument_id, item.exchange_id), &error);
        ASSERT_TRUE(policy.valuation_inputs_verified) << item.exchange_id << ": " << error;
        EXPECT_EQ(policy.generic_close_priority, item.priority);
        EXPECT_EQ(policy.close_rule_source, "reviewed_ctp_close_convention");
        EXPECT_EQ(policy.close_rule_version, "2026-09-07");
        EXPECT_DOUBLE_EQ(policy.open_fee.by_money, 0.001);
        EXPECT_DOUBLE_EQ(policy.close_fee.by_volume, 3);
        EXPECT_DOUBLE_EQ(policy.close_today_fee.by_volume, 4);
        EXPECT_NE(policy.fee_allocation_source.find("generic_close_convention"),
                  std::string::npos);
        EXPECT_NE(policy.valuation_source.find("close_convention_sha256="),
                  std::string::npos);
    }
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       PersistedGenericCloseEvidenceRequiresTheIdenticalCurrentConvention) {
    SimNowBrokerObservedAccountingOptions options;
    options.generic_close_conventions.push_back(GenericConvention("DCE"));
    const AccountingPolicyInstrument scope{"m2701", "DCE", HedgeFlag::kSpeculation};

    VerifiedTradeAccountingPolicyRegistry registry;
    std::string error;
    ASSERT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, options, &error));
    registry.BeginSession(3);
    MakeBrokerObservedEvidence("m2701", "m", "DCE").Observe(&registry);
    ASSERT_TRUE(registry.ReadyFor({scope}, kDay, &error)) << error;
    VerifiedTradeAccountingPolicyRegistry::EvidenceState saved;
    ASSERT_TRUE(registry.ExportValidatedEvidence(&saved, &error)) << error;
    EXPECT_EQ(saved.at("schema_version"), "3");
    ...
    for (int change = 0; change < 4; ++change) {
        ...
        if (change == 0) changed.generic_close_conventions.clear();
        if (change == 1)
            changed.generic_close_conventions[0].priority = GenericClosePriority::kTodayFirst;
        if (change == 2)
            changed.generic_close_conventions[0].evidence_version = "changed-version";
        if (change == 3)
            changed.generic_close_conventions[0].evidence_sha256 =
                "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
        VerifiedTradeAccountingPolicyRegistry mismatch;
        ASSERT_TRUE(mismatch.EnableSimNowBrokerObservedMode(identity, changed, &error));
        EXPECT_FALSE(mismatch.RestoreValidatedEvidence(saved, &error));
    }
}

TEST_F(VerifiedTradeAccountingPolicyTest,
       LegacyExplicitOffsetEvidenceRestoresAndMigratesWithoutOpeningGenericExchanges) {
    VerifiedTradeAccountingPolicyRegistry::EvidenceState legacy = {
        {"schema_version", "2"},
        {"environment", "simnow"},
        ...
        {"count", "1"},
        {"record.0.policy",
         "simnow-broker-observed-v1|6:hc27014:SHFE|0|8:20260906|10|0.001|2|0.002|3|"
         "0.0030000000000000001|4|ctp_verified_zero_v1"},
        {"record.0.instrument", "3:11"},
        {"record.0.commission", "3:12"},
        {"record.0.order_fee", "3:13"},
        {"record.0.instrument_id", "hc2701"},
        {"record.0.exchange_id", "SHFE"},
        {"record.0.hedge_flag", "0"},
        {"record.0.trading_day", "20260906"},
        {"record.0.contract_multiplier", "10"},
        {"record.0.open_by_money", "0.001"},
        {"record.0.open_by_volume", "2"},
        {"record.0.close_by_money", "0.002"},
        {"record.0.close_by_volume", "3"},
        {"record.0.close_today_by_money", "0.003"},
        {"record.0.close_today_by_volume", "4"},
        {"record.0.order_fee_basis", "ctp_verified_zero_v1"},
    };
    VerifiedTradeAccountingPolicyRegistry registry;
    std::string error;
    ASSERT_TRUE(registry.EnableSimNowBrokerObservedMode(identity, {}, &error));
    ASSERT_TRUE(registry.RestoreValidatedEvidence(legacy, &error)) << error;
    EXPECT_TRUE(registry.ResolveHistorical(MatchingHcTrade(), &error).valuation_inputs_verified)
        << error;

    VerifiedTradeAccountingPolicyRegistry::EvidenceState migrated;
    ASSERT_TRUE(registry.ExportValidatedEvidence(&migrated, &error)) << error;
    EXPECT_EQ(migrated.at("schema_version"), "3");
    EXPECT_EQ(migrated.at("record.0.generic_close_priority"), "0");
    EXPECT_EQ(migrated.at("record.0.close_rule_source"), "ctp_explicit_offset");

    auto generic_legacy = legacy;
    generic_legacy["record.0.exchange_id"] = "DCE";
    VerifiedTradeAccountingPolicyRegistry rejected;
    ASSERT_TRUE(rejected.EnableSimNowBrokerObservedMode(identity, {}, &error));
    EXPECT_FALSE(rejected.RestoreValidatedEvidence(generic_legacy, &error));
}
